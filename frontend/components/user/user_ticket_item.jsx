...
class DashboardTicketShow extends React.Component {
	...
	render() {
		...
		const orderValue = Math.round(
			ticket.ticket_price * ticket.ticket_quantity,
			2
		);
				...
				<div className="user-dashboard-ticket-container">
					...
					<section className="user-dashboard-ticket-text-container">
						...
						<div className="user-dashboard-ticket-text-price">
							You spent ${orderValue} for {ticket.ticket_quantity}{' '}
							{pluralizedTickets} on {ticketCreateDate}
						</div>
						...
						<div className="user-dashboard-ownevents-item-actions">
							...
							<button
								className="user-dashboard-ticket-text-delete"
								onClick={this.handleDelete}>
								<i className="fa fa-times" aria-hidden="true" /> Cancel Order
							</button>
						</div>
					</section>
				</div>
